...
/// with a [MdSelectableRegion] widget enables selection within that subtree (for
...
///
/// A [MdSelectableRegion] widget requires configuration; in particular specific
/// [selectionControls] must be provided.
///
/// The [SelectionArea] widget from the [material] library configures a
/// [MdSelectableRegion] in a platform-specific manner (e.g. using a Material
/// toolbar on Android, a Cupertino toolbar on iOS), and it may therefore be
/// simpler to use that widget rather than using [MdSelectableRegion] directly.
///
/// ## An overview of the selection system.
///
/// Every [Selectable] under the [MdSelectableRegion] can be selected. They form a
/// selection tree structure to handle the selection.
///
/// The [MdSelectableRegion] is a wrapper over [SelectionContainer]. It listens to
...
///
/// The [SelectionContainer] created by [MdSelectableRegion] is the root node of a
...
/// a [MdSelectableRegion], consider wrapping that group of widgets using
...
/// wrap part of the subtree with another [MdSelectableRegion]. The selection of the
...
/// See also:
///
///  * [SelectionArea], which creates a [MdSelectableRegion] with
...
class MdSelectableRegion extends StatefulWidget {
  /// Create a new [MdSelectableRegion] widget.
  ///
  /// The [selectionControls] are used for building the selection handles and
  /// toolbar for mobile devices.
  const MdSelectableRegion({
    super.key,
  ...
  ///
  /// By default, [MdSelectableRegion]'s [TextMagnifierConfiguration] is disabled.
  /// For a version of [MdSelectableRegion] that adapts automatically to the
  ...
  final MdSelectableRegionContextMenuBuilder? contextMenuBuilder;
  ...
  ///
  /// For example, [MdSelectableRegion] uses this to generate the default buttons
  /// for its context menu.
  ///
  /// See also:
  ///
  /// * [MdSelectableRegionState.contextMenuButtonItems], which gives the
  ///   [ContextMenuButtonItem]s for a specific MdSelectableRegion.
  ...
  @override
  State<StatefulWidget> createState() => MdSelectableRegionState();
}

/// State for a [MdSelectableRegion].
class MdSelectableRegionState extends State<MdSelectableRegion> with TextSelectionDelegate implements SelectionRegistrar {
  ...
  @override
  void didUpdateWidget(MdSelectableRegion oldWidget) {
  ...
  void _handleFocusChanged() {
    if (!widget.focusNode.hasFocus) {
      ...
      if (SchedulerBinding.instance.lifecycleState == AppLifecycleState.resumed) {
        // We should only clear the selection when this MdSelectableRegion loses
  ...
  void _initTouchGestureRecognizer() {
    ...
    // like PointerDeviceKind.touch so [MdSelectableRegion] gestures do not conflict with
    ...
    _gestureRecognizers[TapAndHorizontalDragGestureRecognizer] = GestureRecognizerFactoryWithHandlers<TapAndHorizontalDragGestureRecognizer>(
        ...
        instance
          ...
          // [MdSelectableRegion] is the child of a horizontal
          // scrollable that means the [MdSelectableRegion] will
  ...
  void _triggerSelectionEndEdgeUpdate({TextGranularity? textGranularity}) {
    ...
    if (_selectable?.dispatchSelectionEvent(
      ...
      }, debugLabel: 'MdSelectableRegion.endEdgeUpdate');
      return;
    }
 }
 ...
 void _onAnyDragEnd(DragEndDetails details) {
  ...
  void _triggerSelectionStartEdgeUpdate({TextGranularity? textGranularity}) {
    ...
    if (_selectable?.dispatchSelectionEvent(
      ...
      }, debugLabel: 'MdSelectableRegion.startEdgeUpdate');
      return;
    }
  }
  ...
  /// Removes the ongoing selection for this [MdSelectableRegion].
  void clearSelection() {
    _finalizeSelection();
  ...
  /// See also:
  ///
  /// * [MdSelectableRegion.getSelectableButtonItems], which performs a similar role,
  ///   but for any selectable text, not just specifically MdSelectableRegion.
  ...
  List<ContextMenuButtonItem> get contextMenuButtonItems {
    return MdSelectableRegion.getSelectableButtonItems(
  ...
  /// Selects the range between two global positions and shows draggable
  /// handles, as if the user had dragged from [start] to [end].
  ///
  /// Added for markdown_widget custom selection mode: lets us programmatically
  /// select a sub-range (e.g. a single element or list item) within the shared
  /// region, after which the user can drag the handles to extend the selection
  /// across element boundaries.
  void selectRange(Offset start, Offset end) {
    clearSelection();
    _selectable?.dispatchSelectionEvent(
  ...
  @override
  void bringIntoView(TextPosition position) {/* MdSelectableRegion must be in view at this point. */}
  ...
  @override
  void userUpdateTextEditingValue(TextEditingValue value, SelectionChangedCause cause) {/* MdSelectableRegion maintains its own state */}
...
class _SelectAllAction extends _NonOverrideAction<SelectAllTextIntent> {
  ...
  final MdSelectableRegionState state;
...
/// [MdSelectableRegionState].
///
/// See also:
///
///  * [EditableTextContextMenuBuilder], which performs the same role for
///    [EditableText].
typedef MdSelectableRegionContextMenuBuilder = Widget Function(
  BuildContext context,
  MdSelectableRegionState selectableRegionState,
);
